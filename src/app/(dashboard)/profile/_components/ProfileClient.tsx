...
export default function ProfileClient({ initialProfileData }: ProfileClientProps) {
  ...
  const getPresetAvatarUrl = () => {
    const avatar = initialProfileData.avatarUrl || '';
    const matchesPreset = PRESETS.some(p => p.url === avatar);
    return matchesPreset ? avatar : PRESETS[0].url;
  };

  const getCustomAvatarUrl = () => {
    const avatar = initialProfileData.avatarUrl || '';
    const matchesPreset = PRESETS.some(p => p.url === avatar);
    return matchesPreset ? '' : avatar;
  };

  const [selectedAvatar, setSelectedAvatar] = useState(getPresetAvatarUrl());
  const [customAvatar, setCustomAvatar] = useState(getCustomAvatarUrl());
  ...
  const [stats, setStats] = useState({
    tripsCreated: initialProfileData.tripsCreated || 0,
    expensesCreated: initialProfileData.expensesCreated || 0
  });
